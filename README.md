# 统一评分引擎 (Unified Stock Scoring Engine)

A股量化评分系统，整合 ISIR / GLM / 豆包 三套评分体系，每日生成综合决策报告。

## 核心功能

### 三套独立评分
- **ISIR** — 31因子 ICIR 原始加权排名（mfi=+0.153, pct_52w=+0.091）
- **GLM** — mfi/pct_52w 方向反转排名（基于 GLM 方向矛盾诊断）
- **豆包** — SS 传统技术评分（技术面35% + 资金面55% + 信息面10%）

### 共识标记
ISIR ∩ GLM ∩ 豆包 三者 TOP30 交集，高亮标记。

### 实战买卖策略
- **买入**：进入 TOP30，T+1 开盘价买入
- **仓位上限**：30 只
- **退出条件**（任一触发）：
  - 时间到期：持仓 10 个交易日
  - 止损：浮亏 ≤ -8%
