# 统一评分引擎 v3.5 (Unified Stock Scoring Engine)
...
- **豆包** — SS 传统技术评分（技术面35% + 资金面55% + 信息面5% + 事件面5%，V9 权重）

### 共识标记 + VCP 确认
ISIR ∩ GLM ∩ 豆包 三者 TOP30 交集，VCP 突破/预突破标注。

> ⚠️ **共识有效性实测（2026-09-22，510只）**：ISIR 与 GLM 仅翻转 `mfi` / `pct_52w` 两个因子，两者排名 Spearman 相关 **0.993**，实质接近同一模型；叠加 SS（信息面+事件面 10% 权重亦为量价衍生）后，「三体系共识」的有效独立票数约为 **2**，门槛低于表面。
...
资金面 = clamp(50 + CMF + 量价共振 + 振幅 + 市值分层 + 主力5日/20日净流向, 5, 95)
信息面 = clamp(50 + 5日涨跌 + 跳空缺口 + 量比 + 连涨天数 + 冲高回落封顶, 5, 95)
事件面 = 25 + clamp(事件分, -25, 25)   # 主流程当前未接入公告/事件源 → 恒为25(中性)
综合SS = 技术面×0.35 + 资金面×0.55 + 信息面×0.05 + 事件面×0.05
```

> 注：SS 的「信息面 + 事件面」合计仅 10% 权重，且信息面由**量价指标衍生**（非独立信息源），与 ISIR/GLM 的技术、资金因子高度重叠——因此「三体系共识」的独立性有限。

## 回测结果（85天，71个有效交易日｜**样本内，含存活偏差**）

### 实战策略

| 策略 | 交易笔数 | 胜率 | 均收益 | 累计收益率* | 均持仓 |
|------|:---:|:---:|:---:|:---:|:---:|
| ISIR 👑 | 380 | 47.1% | +2.0% | +762% | 5.5天 |
| GLM | 503 | 47.1% | +1.8% | +920% | 4.2天 |
| 豆包 | 368 | 44.8% | +1.3% | +462% | 5.7天 |

> ⚠️ **口径说明（2026-09-22 审计后补，务必先读）**
> - **"累计收益率"≠ 组合收益/资金曲线**：该列是每笔交易收益率的**简单相加**（代码 `cum_ret = sum(return_pct)`），如 380 笔 × 均 +2.0% ≈ +762%。它**未做资金复利、未考虑同时持仓的资金占用**，不能当作账户收益率。
> - **未计交易成本**：无手续费/印花税/滑点，未剔除涨停无法买入、停牌、退市样本。
> - **样本内**：ICIR 权重标定的 85 天窗口与回测评估区间**重叠**，属样本内选择，非样本外验证。
> - **存活偏差**：回测用**当前 512 只股票池**回溯历史，而该池经多轮筛选扩张（461→512）才形成，历史上并不存在，收益偏乐观。
> - **可信的部分**：平均单笔收益（+1.3%~+2.0%）、胜率（44.8%~47.1%），以及下方固定持仓基准的 20 日胜率（55.5%~60.2%）。
...
代码与关键数据均在 GitHub 仓库（`mimicangel-png/isir-glm-doubao` 的 main 分支，**当前为公开仓库**），更换 PC 后按以下步骤恢复迭代：
...
- `.workbuddy/` — 完整项目记忆（用户真实持仓与成本、分析方法论约束、每日工作日志、自动化任务执行记录、体系架构决策史）。2026-09-15 起纳入 Git 跟踪；**仓库当前为公开状态，本目录含真实持仓信息**
